...
func (service *authService) IsTaxiTypeValid(taxiType string) bool {
	switch taxiType {
	case "economy":
		return true
...
func (service *authService) RegisterUser(name, phone, email, password, taxiType string) error {
	//Hash
	...
	return service.repository.CreateUser(name, phone, email, hashPass, taxiType)
}
